...
The docs for installing zeek is using ubuntu 24. Because im using ubuntu 22, i need to tweak a few things:
```
...
Another error i found is i need to use Because zeekctl was not in your system PATH, you correctly used:

/opt/zeek/bin/zeekctl

also i did 
Fixing the network interface (MAIN FIX)

You changed Zeek from: (this is because eth0 is not found when i set ip a command)
